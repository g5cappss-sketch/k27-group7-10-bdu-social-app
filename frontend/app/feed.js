    ...
    <>
      <SafeAreaView edges={['top', 'left', 'right']} style={[styles.container, { backgroundColor: '#FFF', flex: 1 }]}>
        ...
        <View style={styles.headerTitleContainer}>
          ...
          <View style={{ 
            flexDirection: 'row', // Nằm ngang để ảnh và chữ sát nhau
            alignItems: 'center'  // Căn giữa theo chiều dọc
          }}>
            
            {/* 1. HÌNH ẢNH CHỮ "B" */}
            <Image 
              source={require('../assets/images/logo-bdu.png.png')} 
              style={{ 
                width: 42,   // Chỉnh lại một chút xíu cho cân đối với chữ
                height: 42 
              }} 
              resizeMode="contain" 
            />

            {/* 2. KHỐI CHỨA CHỮ "DU" VÀ "SOCIAL" */}
            <View style={{ 
              marginLeft: -3,          // Cách chữ B ra một chút cho thoáng
              justifyContent: 'center',
              marginTop: 2            // Đẩy nhẹ cụm chữ xuống để cân bằng với hình
            }}>
              
              {/* Chữ DU to, màu đỏ, in đậm */}
              <Text style={{ 
                fontSize: 27, 
                fontWeight: '900', 
                color: '#fc0707',     // Màu đỏ thương hiệu
                lineHeight: 26,       // Ép chiều cao dòng để chữ Social xích lại gần
                letterSpacing: -0.5 ,  // Kéo 2 chữ D và U sát lại nhìn cho mạnh mẽ
              ...
              {/* Chữ SOCIAL nhỏ hơn, nằm dưới */}
              <Text style={{ 
                fontSize: 11, 
                fontWeight: '700', 
                color: '#65676B',     // Màu xám đậm chuyên nghiệp
                letterSpacing: 1.5,   // Kéo giãn các chữ cái ra nhìn rất sang
                marginTop: -1         // Kéo nhích lên cho sát mí chữ DU
              }}>
                SOCIAL
              </Text>
...
<View style={styles.topMenuBar}>
  ...
  >
    <Ionicons name={activeTab === 'feed' ? "home" : "home-outline"} size={26} color={activeTab === 'feed' ? BDU_RED : "#65676B"} />
  </TouchableOpacity>
  ...
  <TouchableOpacity 
    ...
    activeOpacity={0.7}
    onPress={() => { setActiveTab('match'); router.replace('/match'); }}
  >
    <Ionicons name={activeTab === 'match' ? "people" : "people-outline"} size={26} color={activeTab === 'match' ? BDU_RED : "#65676B"} />
  </TouchableOpacity>
  ...
  <TouchableOpacity 
    ...
    activeOpacity={0.7}
    onPress={() => { setActiveTab('notification'); setNotificationModalVisible(true); }}
  >
    <View style={{ position: 'relative' }}>
      <Ionicons name={activeTab === 'notification' ? "notifications" : "notifications-outline"} size={26} color={activeTab === 'notification' ? BDU_RED : "#65676B"} />
      {/* Badge đếm số thông báo */}
  ...
  >
    <Ionicons name={activeTab === 'profile' ? "person" : "person-outline"} size={26} color={activeTab === 'profile' ? BDU_RED : "#65676B"} />
  </TouchableOpacity>
  ...
  <TouchableOpacity 
    ...
    activeOpacity={0.7}
    onPress={() => { setActiveTab('menu'); setMenuVisible(true); }}
  >
    <Ionicons name={activeTab === 'menu' ? "menu" : "menu-outline"} size={28} color={activeTab === 'menu' ? BDU_RED : "#65676B"} />
  </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  topMenuBar: {
    flexDirection: 'row',
    backgroundColor: '#FFF',
    // Giữ lại một đường viền xám siêu mờ ở dưới cùng để tách biệt với phần bài viết
    borderBottomWidth: 1,
    borderBottomColor: '#E4E6EB', 
    
    // Đưa phần đổ bóng (shadow) xuống đây để bọc luôn cả cụm Header + TabBar
    elevation: 4,
    shadowColor: '#000',
    shadowOffset: { width: 0, height: 2 },
    shadowOpacity: 0.05,
    shadowRadius: 3, // Vạch kẻ mờ dưới cùng
  },
  menuTabItem: {
    flex: 1, // Chia đều 5 tab
    alignItems: 'center',
    justifyContent: 'center',
    paddingVertical: 12,
  },
  menuTabItemActive: {
    borderBottomWidth: 3, // Gạch chân đỏ khi active
    borderBottomColor: BDU_RED, 
  },
  badgeNumber: {
    position: 'absolute',
    top: -4,
 ...
 // --- BÀI VIẾT ĐANG DÙNG CLASS NÀY ---
  postCard: {
    ...
    // 🔴 CHỈNH SỬA TẠI ĐÂY:
    borderWidth: 1.5,       // Đổi thành 1.5 cho rõ viền
    borderColor: '#ff4a4a', // 🔴 ĐỔI TỪ '#E4E6EB' THÀNH MÀU ĐỎ NÀY!
